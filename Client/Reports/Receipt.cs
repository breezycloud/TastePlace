...
public class Receipt
{
    ...
    void ComposeHeader(IContainer container)
    {
        container.AlignCenter().Column(column => 
        {
            ...
            column.Item().AlignCenter().Text("TastePlace").Bold().FontSize(20);
            column.Item().AlignCenter().Text("Order Receipt").FontSize(15);
    ...
    void ComposeContent(IContainer container)
    {
        ...
        {      
            column.Item().Row(row => 
            {
                row.RelativeItem().AlignLeft().Text($"{Order!.Customer!.FullName}").FontSize(9);
    ...
    {        
        ...
        container.PaddingVertical(1.2f).Padding(1.2f).Table(table =>
        {
            ...
            foreach (var item in Model!)
            {
                ...
                table.Cell().Text(item.Item!.MenuName).FontSize(9);
    ...
    {        
        return Order!.Payments!.OrderByDescending(d => d.ModifiedTicks)
                                           .Take(1)
                                           .Select(x => x.Amount)
                                           .FirstOrDefault(0)
                                           .ToString("N2");
    ...
    {
        return (Order!.SubTotal - Order!.Payments!.Sum(x => x.Amount)).ToString("N2");        
    }
}